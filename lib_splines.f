...
contains
  ...
  ! boundary conditions specified by bcytpe, bcvals
    ...
    if (bctype(1) < 1 .or. bctype(1) > 2) call error('spline3pars', 'bctype /= 1 or 2.')
    if (bctype(2) < 1 .or. bctype(2) > 2) call error('spline3pars', 'bctype /= 1 or 2.')
    if (size(c,1) /= 5) call error('spline3pars', 'size(c,1) /= 5.')
    if (size(c,2) /= size(xi)-1) call error('spline3pars', 'size(c,2) /= size(xi)-1.')
    if (size(xi) /= size(yi)) call error('spline3pars', 'size(xi) /= size(yi)')
    ...
    ! compute interpolating-cubic 2nd derivs at ends, if required
       ! left end
    if(bctype(1)==2) then
    if (n < 4) call error('spline3pars', 'n < 4')
       xe=xi(1:4)
       ye=yi(1:4)
    ...
    Ae(:,1) = 1 ! set 0^0 = 1
       ...
       if (info /= 0) call error('spline3pars', 'dgesv error.')
       ce=bemat(:,1)
       d2p1=2*ce(3)
    end if
       ! right end
    ...
    if (info /= 0) call error('spline3pars', 'dgbsv error.')
    cs=bmat(:,1)
    !write(*,*) cs(1:6)
...
!--------------------------------------------------------------------------------------------------!
  ...
  ! returned by spline3pars
    ...
    n=size(xi)
    if (size(c,1) /= 5) call error('spline3', 'size(c,1) /= 5.')
    if (size(c,2) /= size(xi)-1) call error('spline3', 'size(c,2) /= size(xi)-1.')
