...
!  An additional subroutine generates a random ordering of the integers 1 .. N
!   random_order
 
!  The generators are seeded by the routine (works with any compiler?)
!   seed_random_number
...
!  The intrinsic random number generator, RANDOM_NUMBER(r), is used to provide
!  uniformly distributed random numbers.

!  At this stage, only one random number is generated at each call.

!  The module also uses the following functions (which are included):
...
PRIVATE            :: integral
integer,private,parameter :: dp = selected_real_kind(p=15,r=307)
...
function random_uniform_dp(a, b) result(fn_val)
! This function returns a uniformly distributed pseudo-random number
! between a and b. Double precision version
...
end function random_uniform_dp

function random_uniform_sp(a, b) result(fn_val)
! This function returns a uniformly distributed pseudo-random number
! between a and b. Single precision version
...
end function random_uniform_sp
...
!-----------------------------
! Version 1.14, 28 November 2012
! Changes:
! 1. The routine seed_random_number has been changed. Now it seeds the uniform
!	 random number generator with the system time and does not require input
!	 from the user. (Joao Faria)
